      ...
      if target_recurrence <> 'none' then
        ...
        if target_recurrence = 'daily' then
          ...
          next_local_due_at := date_trunc('month', next_local_due_at + interval '1 month')
            + (least(
              coalesce(target_recurrence_day, extract(day from next_local_due_at)::integer),
...
grant insert (recurrence_day) on table tasks to authenticated;
grant update (recurrence_day) on table tasks to authenticated;